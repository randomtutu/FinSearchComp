...
class DeepSeekChat:
    ...
    def __call__(self, prompts, histories, **kwargs):
        ...
        for prompt, history in zip(prompts, histories):
            ...
            logger.debug("Sending request to DeepSeek model %s with %d messages", self.model_name, len(messages))
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.error("DeepSeek chat request failed: %s", error_msg)
                    responses.append({"error": error_msg})
                    
            except Exception as e:
                logger.error("DeepSeek chat request raised exception: %s", e)
